...
- **Minimize-mode (chạy chiến dịch khi app đang minimize)** — combo 2 lớp giảm trigger Win32 auto-restore:
  1. `wc.setBackgroundThrottling(false)` ([browserHandlers.ts:23](src/main/ipc/handlers/browserHandlers.ts:23)) khi register webview → giữ timer Chromium chạy đầy đủ.
  2. `wc.executeJavaScript(code, false)` ([pageController.ts:117](src/main/v2/runtime/pageController.ts:117), [:455](src/main/v2/runtime/pageController.ts:455)) — userGesture=false để Chromium không kích hoạt host window. KHÔNG dùng `.focus()` trên DOM trong webview JS — dùng `dispatchEvent(new FocusEvent("focusin", { bubbles: true, cancelable: true, view: window }))` (FB lazy-load + Lexical observer chỉ cần focusin event; `execCommand('insertText'/'insertParagraph')` chạy trên Selection hiện tại, không cần `document.activeElement`). Xem [pageController.ts](src/main/v2/runtime/pageController.ts) `type()`/`fill()`/`hover()`.
  Note: Chromium internal (`loadURL`/CDP/paint) vẫn có thể request window activation. Nếu cần hard-prevent restore, thêm minimize watcher ở main process — track `userMinimized` flag, trên `'restore'` event re-minimize qua `setImmediate(() => mainWindow.minimize())`. KHÔNG để mặc định vì sẽ chặn user click taskbar restore khi run đang chạy.
