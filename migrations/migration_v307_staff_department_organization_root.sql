-- v307: share Chat's single organization-root model.
-- Built from live pg_get_functiondef on cgjbsmqtfhqvttudyjzq (source 45158ff8555a5a12501949371d08b658).
-- Body only: preserve signature, return type, owner, ACL and all staff/expiry/device guards.
BEGIN;
SET LOCAL lock_timeout='3s';
SET LOCAL statement_timeout='15s';
DO $preflight$ DECLARE r record; BEGIN
 SELECT md5(pg_get_functiondef(oid)) checksum,jsonb_build_object('owner',pg_get_userbyid(proowner),'prosecdef',prosecdef,'provolatile',provolatile,'proconfig',proconfig,'proacl',proacl::text) attrs INTO r FROM pg_proc WHERE oid=to_regprocedure('public.aka_agent_staff_management(bigint,text,text,text,jsonb)');
 IF NOT FOUND OR r.checksum NOT IN ('45158ff8555a5a12501949371d08b658','182c5e461d34e1028e5c6e19e32abe00') OR r.attrs IS DISTINCT FROM '{"owner": "postgres", "prosecdef": true, "provolatile": "v", "proconfig": ["search_path=pg_catalog, public", "lock_timeout=3s", "statement_timeout=12s"], "proacl": "{postgres=X/postgres,anon=X/postgres,authenticated=X/postgres,service_role=X/postgres}"}'::jsonb THEN RAISE EXCEPTION 'v307 staff management live definition/attribute drift'; END IF;
END; $preflight$;
...
BEGIN
  ...
  IF mutation THEN
    ...
    IF p_action='saveGroup' THEN
      -- Same lock as Chat's ensureRootDepartment, before any actor/group row lock.
...
-- No explicit schema reload: API metadata is unchanged. Existing DDL watchers stay enabled.
COMMIT;
