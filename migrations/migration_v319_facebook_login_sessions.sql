...
-- Captured from linked akachat. Existing helpers are dependencies, never replaced.
DO $$ BEGIN
  IF md5(pg_get_functiondef('public.aka_agent_staff_access(bigint,text,text)'::regprocedure)) <> 'ca59bfbfa79c7e40ec50788e77b9d022' THEN
    RAISE EXCEPTION 'facebook_staff_access_preflight_changed';
  END IF;
  IF to_regprocedure('public.aka_agent_facebook_login(bigint,text,text,text,jsonb)') IS NOT NULL
     OR to_regprocedure('public.aka_agent_facebook_secret_cleanup()') IS NOT NULL
     OR to_regprocedure('public.aka_agent_facebook_account_cleanup()') IS NOT NULL
     OR to_regclass('public.auto_facebook_login_imports') IS NOT NULL
     OR to_regclass('public.auto_facebook_login_sessions') IS NOT NULL THEN
    RAISE EXCEPTION 'facebook_migration_already_present_inspect_live_definition';
  END IF;
END $$;

ALTER TABLE public.auto_accounts
  ADD COLUMN facebook_uid text,
  ADD COLUMN facebook_login_managed boolean NOT NULL DEFAULT false;
CREATE INDEX idx_auto_accounts_facebook_uid ON public.auto_accounts(staff_id, facebook_uid)
  WHERE flatform_type='facebook' AND is_delete=false;

CREATE TABLE public.auto_facebook_login_sessions (
  account_id bigint PRIMARY KEY REFERENCES public.auto_accounts(id) ON DELETE CASCADE,
...
ALTER TABLE public.auto_facebook_login_sessions ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.auto_facebook_login_sessions FROM PUBLIC, anon, authenticated;

-- A reservation allocates an ID, never an account or a secret. It allows profile promotion
-- before the single final account transaction and avoids expensive account FK cleanup.
CREATE TABLE public.auto_facebook_login_imports (
...
CREATE INDEX idx_facebook_login_imports_staff ON public.auto_facebook_login_imports(staff_id);
ALTER TABLE public.auto_facebook_login_imports ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.auto_facebook_login_imports FROM PUBLIC,anon,authenticated;

INSERT INTO public.auto_system_settings(key,value,description,is_secret,is_active)
VALUES ('facebook.account_import.max_accounts_per_batch','10','Số tài khoản Facebook đầu vào tối đa mỗi lượt import.',false,true)
ON CONFLICT(key) DO NOTHING;

CREATE FUNCTION public.aka_agent_facebook_secret_cleanup() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
BEGIN DELETE FROM vault.secrets WHERE id=OLD.secret_id; RETURN OLD; END $$;
REVOKE ALL ON FUNCTION public.aka_agent_facebook_secret_cleanup() FROM PUBLIC,anon,authenticated;
CREATE TRIGGER trg_facebook_secret_cleanup AFTER DELETE ON public.auto_facebook_login_sessions
FOR EACH ROW EXECUTE FUNCTION public.aka_agent_facebook_secret_cleanup();

CREATE FUNCTION public.aka_agent_facebook_account_cleanup() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
BEGIN
  IF NEW.is_delete IS TRUE OR NEW.flatform_type<>'facebook'
     OR NEW.staff_id IS DISTINCT FROM OLD.staff_id OR NEW.organization_id IS DISTINCT FROM OLD.organization_id THEN
    DELETE FROM public.auto_facebook_login_sessions WHERE account_id=OLD.id;
    NEW.facebook_login_managed:=false;
  END IF;
  RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION public.aka_agent_facebook_account_cleanup() FROM PUBLIC,anon,authenticated;
CREATE TRIGGER trg_facebook_account_cleanup BEFORE UPDATE OF is_delete,flatform_type,staff_id,organization_id
ON public.auto_accounts FOR EACH ROW EXECUTE FUNCTION public.aka_agent_facebook_account_cleanup();

CREATE FUNCTION public.aka_agent_facebook_login(
  p_staff_id bigint,p_username text,p_password text,p_action text,p_data jsonb DEFAULT '{}'
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public SET statement_timeout='15s' AS $$
DECLARE
...
END $$;
REVOKE ALL ON FUNCTION public.aka_agent_facebook_login(bigint,text,text,text,jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.aka_agent_facebook_login(bigint,text,text,text,jsonb) TO anon,authenticated,service_role;
COMMENT ON FUNCTION public.aka_agent_facebook_login(bigint,text,text,text,jsonb) IS
  'Credential-checked Facebook import/session API. Duplicate UID filtering is exclusive to bulk import; secrets never returned in metadata.';
