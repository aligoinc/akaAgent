-- Zalo API-only campaign: message/add friend to scanned group members.
-- Targets are selected from zalo_group_members mirrored into auto_account_contacts.
...
VALUES
  (
    'zalo_resolve_group_member_target',
    'Chuẩn bị target Zalo từ UID thành viên group đã quét.',
    'UserSearch',
    'data',
    'js',
    $block$
return await helpers.zaloResolveGroupMemberTarget({
...
$block$,
  ...
  (
    'zalo_send_group_member_message',
    'Gửi tin nhắn/attachment Zalo đến thành viên group đã quét.',
    'MessageCircle',
    'data',
    'js',
    $block$
...
$block$,
  ...
  (
    'zalo_send_group_member_friend_request',
    'Gửi lời mời kết bạn Zalo đến thành viên group đã quét.',
    'UserPlus',
    'data',
    'js',
    $block$
...
BEGIN
  ...
  IF resolve_block_id IS NULL OR message_block_id IS NULL OR friend_block_id IS NULL OR tag_block_id IS NULL OR alias_block_id IS NULL THEN
    RAISE EXCEPTION 'Cannot seed zalo_message_group_member workflow: missing Zalo block ids';
  END IF;
  ...
  VALUES (
    ...
    'Workflow browserless cho chiến dịch Zalo - Nhắn tin, kết bạn đến thành viên group.',
    jsonb_build_array(
  ...
  SELECT
    'zalo_message_group_member__test__zalo_message_group_member',
    description,
    nodes,
    edges,
    variables_schema,
  ...
  FROM public.auto_workflows
  WHERE name = 'zalo_message_group_member__test__zalo_message_group_member';
  ...
  VALUES (
    'zalo_message_group_member',
    'Zalo - Nhắn tin, kết bạn đến thành viên group',
    'zalo',
    true,
    workflow_id,
    test_workflow_id,
