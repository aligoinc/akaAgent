-- v302: Test-only Facebook post media upload via the composer's real file input.
-- Source: linked production cgjbsmqtfhqvttudyjzq, captured 2026-09-21.
-- Changes only codeOverride on one node in each test workflow 250/251/252.
-- Shared blocks, real workflows, elements, action mappings and runtime stay unchanged.
-- Data only: no RPC/schema changes, auxiliary DDL or PostgREST schema reload.
BEGIN;
...
DECLARE
  ...
  "workflows": [
    ...
    },
    {
      ...
      "source_md5": "ede59caf8e4ab0dd439322831730cd87",
    ...
    },
    {
      ...
      "source_md5": "a7c44d5cd852f6f3efd307f5eab59907",
      "node_id": "drop_images",
      "block_id": 29
    },
    {
      ...
      "source_md5": "ae44be488656b7a692b94b574ecf982d",
      "node_id": "drop_images",
      "block_id": 29
    }
  ],
  ...
  v_helper text := $upload_helper$// Test override only: use the composer's real file input, never drag/drop.
...
async function uploadPostMediaInput(formXPath, files) {
  ...
  v_count integer;
BEGIN
  -- Lock source/configuration rows in a fixed order before checking drift.
  PERFORM 1 FROM public.auto_blocks WHERE id IN (29,2672) ORDER BY id FOR SHARE;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(v_patch->'blocks') LOOP
    ...
    IF v_row IS NULL OR md5(v_row::text) IS DISTINCT FROM v_item->>'source_md5'
      OR v_row->>'code' IS DISTINCT FROM v_item->>'source_code' THEN
      RAISE EXCEPTION 'v302 source block drift: %',v_item->>'name';
    END IF;
    IF cardinality(string_to_array(v_row->>'code',v_item->>'before')) <> 2 THEN
      RAISE EXCEPTION 'v302 expected exactly one upload replacement: %',v_item->>'name';
    END IF;
    v_code := replace(v_row->>'code',v_item->>'before',v_item->>'after') || E'\n' || v_helper || E'\n';
    IF md5(v_code) IS DISTINCT FROM v_item->>'override_md5' THEN
      RAISE EXCEPTION 'v302 override checksum mismatch: %',v_item->>'name';
    END IF;
    v_codes := v_codes || jsonb_build_object(v_item->>'id',v_code);
  END LOOP;
  ...
  IF v_row IS NULL OR md5(v_row::text) IS DISTINCT FROM v_patch#>>'{element,source_md5}' THEN
    RAISE EXCEPTION 'v302 FbComposerForm drift';
  END IF;
  FOR v_item IN SELECT value FROM jsonb_array_elements(v_patch->'actions') LOOP
    IF NOT EXISTS (SELECT 1 FROM public.auto_campaign_actions a
      WHERE a.id=v_item->>'id' AND a.workflow_id=(v_item->>'workflow_id')::bigint
      AND a.test_workflow_id=(v_item->>'test_workflow_id')::bigint
      AND a.workflow_id IS DISTINCT FROM a.test_workflow_id) THEN
      RAISE EXCEPTION 'v302 real/test action mapping drift: %',v_item->>'id';
    END IF;
  END LOOP;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(v_patch->'workflows') LOOP
    ...
    IF v_row IS NULL OR md5(v_row::text) IS DISTINCT FROM v_item->>'source_md5' THEN
      RAISE EXCEPTION 'v302 workflow drift: %',v_item->>'name';
    END IF;
  END LOOP;

  FOR v_item IN SELECT value FROM jsonb_array_elements(v_patch->'workflows') WHERE value ? 'node_id' LOOP
    SELECT to_jsonb(w) INTO v_before FROM public.auto_workflows w WHERE w.id=(v_item->>'id')::bigint;
    SELECT count(*) INTO v_count FROM jsonb_array_elements(v_before->'nodes') n
      WHERE n->>'id'=v_item->>'node_id' AND n->>'blockId'=v_item->>'block_id'
      AND n->>'codeOverride' IS NULL;
    IF v_count <> 1 THEN RAISE EXCEPTION 'v302 missing or overridden test node: %',v_item->>'name'; END IF;

    SELECT jsonb_agg(CASE WHEN n->>'id'=v_item->>'node_id'
      THEN jsonb_set(n,'{codeOverride}',v_codes->(v_item->>'block_id'),true) ELSE n END ORDER BY ord)
    ...
    SELECT to_jsonb(w) INTO v_row FROM public.auto_workflows w WHERE w.id=(v_item->>'id')::bigint;
    IF (v_row - 'nodes' - 'updated_at') IS DISTINCT FROM (v_before - 'nodes' - 'updated_at')
      OR v_row->'nodes' IS DISTINCT FROM v_nodes THEN
      RAISE EXCEPTION 'v302 workflow postflight failed: %',v_item->>'name';
    END IF;
  END LOOP;

  -- Assert shared production definitions are byte-for-byte unchanged.
  FOR v_item IN SELECT value FROM jsonb_array_elements(v_patch->'workflows') WHERE NOT value ? 'node_id' LOOP
    IF NOT EXISTS (SELECT 1 FROM public.auto_workflows w WHERE w.id=(v_item->>'id')::bigint
      ...
      RAISE EXCEPTION 'v302 real workflow changed: %',v_item->>'name';
    END IF;
  END LOOP;
  FOR v_item IN SELECT value FROM jsonb_array_elements(v_patch->'blocks') LOOP
    IF NOT EXISTS (SELECT 1 FROM public.auto_blocks b WHERE b.id=(v_item->>'id')::bigint
      AND md5(to_jsonb(b)::text)=v_item->>'source_md5') THEN
      RAISE EXCEPTION 'v302 shared block changed: %',v_item->>'name';
    END IF;
  END LOOP;
END;
$post_upload_test$;
