-- Zalo API-only campaign: message/add friend to customers previously messaged by campaigns.
-- Targets are selected dynamically from auto_campaign_details history.
...
VALUES
  (
    'zalo_resolve_remarketing_customer_target',
    'Chuẩn bị target Zalo từ UID khách hàng cũ từng gửi tin.',
    'UserSearch',
    'data',
    'js',
    $block$
return await helpers.zaloResolveRemarketingCustomerTarget({
...
$block$,
  ...
  (
    'zalo_send_remarketing_customer_message',
    'Gửi tin nhắn/attachment Zalo đến khách hàng cũ từng gửi tin.',
    'MessageCircle',
    'data',
    'js',
    $block$
...
$block$,
  ...
  (
    'zalo_send_remarketing_customer_friend_request',
    'Gửi lời mời kết bạn Zalo đến khách hàng cũ từng gửi tin.',
    'UserPlus',
    'data',
    'js',
    $block$
...
BEGIN
  SELECT id INTO resolve_block_id FROM public.auto_blocks WHERE name = 'zalo_resolve_remarketing_customer_target';
  SELECT id INTO message_block_id FROM public.auto_blocks WHERE name = 'zalo_send_remarketing_customer_message';
  SELECT id INTO friend_block_id FROM public.auto_blocks WHERE name = 'zalo_send_remarketing_customer_friend_request';
  ...
  IF resolve_block_id IS NULL OR message_block_id IS NULL OR friend_block_id IS NULL OR tag_block_id IS NULL OR alias_block_id IS NULL THEN
    RAISE EXCEPTION 'Cannot seed zalo_message_remarketing_customer workflow: missing Zalo block ids';
  END IF;
  ...
  VALUES (
    ...
    'Workflow browserless cho chiến dịch Zalo - Nhắn tin, kết bạn đến khách hàng cũ từng gửi tin.',
    jsonb_build_array(
      jsonb_build_object('id','resolve_customer','blockId',resolve_block_id,'blockName','zalo_resolve_remarketing_customer_target','position',jsonb_build_object('x',0,'y',0),'config',jsonb_build_object()),
      jsonb_build_object('id','send_message','blockId',message_block_id,'blockName','zalo_send_remarketing_customer_message','position',jsonb_build_object('x',260,'y',0),'config',jsonb_build_object()),
      jsonb_build_object('id','send_friend_request','blockId',friend_block_id,'blockName','zalo_send_remarketing_customer_friend_request','position',jsonb_build_object('x',520,'y',0),'config',jsonb_build_object()),
    ...
    jsonb_build_array(
      jsonb_build_object('id','e_resolve_message','source','resolve_customer','target','send_message'),
  ...
  SELECT
    'zalo_message_remarketing_customer__test__zalo_message_remarketing_customer',
    description,
    nodes,
    edges,
    variables_schema,
  ...
  FROM public.auto_workflows
  WHERE name = 'zalo_message_remarketing_customer__test__zalo_message_remarketing_customer';
  ...
  VALUES (
    'zalo_message_remarketing_customer',
    'Zalo - Nhắn tin, kết bạn đến khách hàng cũ từng gửi tin',
    'zalo',
    true,
    workflow_id,
    test_workflow_id,
