-- Rollback v323 data only. Do not alter existing locks or restart campaigns.
BEGIN;
...
UPDATE public.auto_error e SET
  is_active=s.is_active,
  is_delete=s.is_delete,
  created_at=s.created_at,
  error_code=s.error_code,
  error_desc=s.error_desc,
  error_name=s.error_name,
  error_type=s.error_type,
  updated_at=s.updated_at,
  detail_status=s.detail_status,
  error_element=s.error_element,
  noti_campaign=s.noti_campaign,
  zalo_error_codes=s.zalo_error_codes,
  zalo_action_codes=s.zalo_action_codes,
  counts_toward_limit=s.counts_toward_limit,
  disable_action_mode=s.disable_action_mode,
  update_login_status=s.update_login_status,
  disable_action_codes=s.disable_action_codes,
  noti_running_process=s.noti_running_process,
  time_disable_actions=s.time_disable_actions,
  update_status_account=s.update_status_account,
  update_status_campaign=s.update_status_campaign,
  count_consecutive_errors=s.count_consecutive_errors,
  counts_toward_bad_target=s.counts_toward_bad_target,
  disable_action_days=NULL, disable_action_time=NULL
FROM jsonb_populate_recordset(NULL::public.auto_error, $json$
[
  {
    "id": 12,
    "is_active": true,
...
$json$::jsonb) s WHERE e.id=s.id;
UPDATE public.auto_error SET is_active=false, updated_at=transaction_timestamp()
WHERE error_code IN (SELECT x->>'error_code' FROM jsonb_array_elements($json$
[
  {
    "error_code": "err_zalo_120_message_stranger",
    "error_type": "external zalo",
...
$json$::jsonb;
BEGIN
  IF EXISTS (SELECT 1 FROM public.auto_error e WHERE expected ? e.id::text AND
    expected->>e.id::text IS DISTINCT FROM md5((to_jsonb(e)-'disable_action_days'-'disable_action_time')::text)) THEN
    RAISE EXCEPTION 'v323 rollback: original policies differ from snapshot';
  END IF;
END $rollback_verify$;
COMMIT;
