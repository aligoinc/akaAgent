-- v274: retain runtime/ownership checks, information edits and reimport/retry.
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'v274_smoke: active fixture tenant/action missing';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM public.auto_account_contacts WHERE id=8800274000000900)
    OR EXISTS (SELECT 1 FROM public.auto_account_contact_group_members WHERE id=8800274000000901)
  THEN RAISE EXCEPTION 'v274_smoke: source fixture collision'; END IF;
  ...
  OVERRIDING SYSTEM VALUE VALUES
    (8800274000000900,NULL,'person','__v274_source__','0900000000','zalo',v_staff_id,v_organization_id,false);
  INSERT INTO public.auto_account_contact_group_members(id,group_id,contact_id,is_delete)
  OVERRIDING SYSTEM VALUE VALUES (8800274000000901,v_group_id,8800274000000900,false);

  FOREACH v_test_campaign IN ARRAY ARRAY[v_local_campaign_id,v_server_campaign_id] LOOP
    v_result:=public.aka_agent_snapshot_data_group_to_direct_campaign(
      v_staff_id,v_organization_id,'__v274_first__:'||v_test_campaign,v_test_campaign,v_group_id,
      now(),'tạm dừng',v_auth_username,v_auth_password);
    IF (v_result->>'inserted_count')::int IS DISTINCT FROM 1
    THEN RAISE EXCEPTION 'v274_smoke: first import failed: %',v_result; END IF;
    ...
    v_result:=public.aka_agent_snapshot_data_group_to_direct_campaign(
      v_staff_id,v_organization_id,'__v274_repeat__:'||v_test_campaign,v_test_campaign,v_group_id,
      now(),'tạm dừng',v_auth_username,v_auth_password);
    ...
    THEN RAISE EXCEPTION 'v274_smoke: reimport overwrote metadata or lost dedupe'; END IF;
    ...
    END;
    IF NOT v_rejected
      OR NOT EXISTS (SELECT 1 FROM public.auto_campaign_input_data WHERE id=v_input_id
        AND phone='0900000000' AND canonical_target_key=v_target_key)
      OR NOT EXISTS (SELECT 1 FROM public.auto_campaign_input_target_aliases WHERE input_data_id=v_input_id)
    ...
    THEN RAISE EXCEPTION 'v274_smoke: target/key/source protection failed'; END IF;
    v_replayed:=public.aka_agent_snapshot_data_group_to_direct_campaign(
      v_staff_id,v_organization_id,'__v274_repeat__:'||v_test_campaign,v_test_campaign,v_group_id,
      now(),'tạm dừng',v_auth_username,v_auth_password);
    IF v_replayed IS DISTINCT FROM v_result
      OR (SELECT count(*) FROM public.auto_campaign_input_data WHERE campaign_id=v_test_campaign AND is_delete=false)<>1
    THEN RAISE EXCEPTION 'v274_smoke: request replay lost idempotency'; END IF;
  END LOOP;

END;
$behavior$;

SELECT 'PASS: local/server snapshot, metadata retention, target-edit rejection, origin preservation, idempotent retry and ownership guards' AS result;
ROLLBACK;
