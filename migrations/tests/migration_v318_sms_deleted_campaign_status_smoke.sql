-- Run with scripts/run-sms-deleted-campaign-smoke.cjs --v318 in an isolated local DB.
BEGIN;
...
UPDATE auto_campaigns SET account_id=30 WHERE id=1;
DO $test$
...
BEGIN
  ...
  SELECT * INTO r FROM aka_agent_record_sms_message_status(1,20,'đã gửi');
  ASSERT r.accepted AND NOT r.counted, 'B may report A SMS after reassignment';
  ASSERT (SELECT account_id=10 FROM auto_campaign_details WHERE input_data_id=1), 'original owner retained';
  SELECT * INTO r FROM aka_agent_record_sms_message_status(5,10,'đã nhận');
  ASSERT r.accepted AND NOT r.counted;
  ASSERT (SELECT account_id=20 FROM auto_campaign_details WHERE input_data_id=5), 'history owner unchanged';
  SELECT * INTO r FROM aka_agent_record_sms_message_status(9,10,'đã nhận');
  ASSERT r.accepted AND NOT r.counted;
  ASSERT (SELECT campaign_id=2 FROM auto_campaign_details WHERE input_data_id=9), 'history campaign unchanged';
  ...
  SELECT * INTO r FROM aka_agent_record_sms_message_status(2,20,'đã nhận');
  ASSERT NOT r.accepted AND r.message='Chưa có trạng thái đã gửi để cập nhật đã nhận', 'existing prerequisite unchanged';
  -- The current APK already recovers a missing sent report before retrying delivery.
  SELECT * INTO r FROM aka_agent_record_sms_message_status(2,20,'đã gửi');
  ASSERT r.accepted AND r.counted AND NOT r.input_updated, 'sent report accepted after deletion';
  SELECT * INTO r FROM aka_agent_record_sms_message_status(2,20,'đã nhận');
  ASSERT r.accepted AND NOT r.counted AND NOT r.input_updated;
  SELECT * INTO r FROM aka_agent_record_sms_message_status(2,10,'đã nhận');
  ASSERT r.accepted AND NOT r.counted;
  ASSERT (SELECT count(*)=1 FROM auto_campaign_details WHERE input_data_id=2);
  ASSERT (SELECT account_id=20 FROM auto_campaign_details WHERE input_data_id=2);
  ASSERT (SELECT value=1 FROM test_counts WHERE account_id=20);
  ...
  ASSERT NOT EXISTS(SELECT 1 FROM test_counts WHERE account_id=10), 'existing history must not count quota';
  ASSERT (SELECT value=1 FROM test_counts WHERE account_id=20), 'only first new history counted';
  ...
  SELECT * INTO r FROM aka_agent_record_sms_message_status(4,20,'đã nhận');
  ASSERT r.accepted AND NOT r.counted AND NOT r.input_updated;
  ...
  SELECT * INTO r FROM aka_agent_record_sms_message_status(3,10,'đã gửi');
  ASSERT r.accepted AND NOT r.counted;
  SELECT * INTO r FROM aka_agent_record_sms_message_status(3,20,'đã nhận');
  ASSERT (SELECT account_id=10 FROM auto_campaign_details WHERE input_data_id=3);
...
END;
$test$;
SELECT 'v318 SMS status smoke PASS' AS result;
ROLLBACK;
