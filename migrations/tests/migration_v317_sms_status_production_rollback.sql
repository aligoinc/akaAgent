...
DO $smoke$
DECLARE
  r record; before_campaign jsonb; before_input jsonb; before_counter jsonb;
...
BEGIN
  ...
  SELECT to_jsonb(s) INTO before_counter FROM public.auto_account_action_status s
    WHERE account_id=4745 AND action_code='sms_send';
  ...
  SELECT * INTO r FROM public.aka_agent_record_sms_message_status(4486246,-317,'đã nhận');
  ASSERT NOT r.accepted, 'wrong account must still fail';
  ...
  SELECT * INTO r FROM public.aka_agent_record_sms_message_status(4486246,4745,'đã nhận','v317 rollback smoke');
  ...
  ASSERT before_counter IS NOT DISTINCT FROM (SELECT to_jsonb(s) FROM public.auto_account_action_status s WHERE account_id=4745 AND action_code='sms_send'), 'quota changed';
...
$smoke$;
RESET ROLE;
SELECT 'v317 production rollback PASS' AS result;
ROLLBACK;
