...
DECLARE
  ...
  r jsonb; payload jsonb; request_id uuid:=gen_random_uuid(); vault_id uuid; rev bigint; claim_token uuid:=gen_random_uuid(); enrollment_id bigint; other_actor bigint; other_name text; total_accounts bigint;
BEGIN
  ...
  INSERT INTO public.aka_customer(name,phone) VALUES('__facebook_v320_rollback__','0999999320') RETURNING id INTO customer_id;
  INSERT INTO public.org_organization(customer_id,name,phone,max_staff,staff_id_created,staff_id_owner)
    SELECT customer_id,'__facebook_v320_rollback__','0999999320',3,id,id FROM public.org_staff WHERE organization_id=1 AND is_admin IS TRUE LIMIT 1 RETURNING id INTO org_id;
  INSERT INTO public.org_staff(organization_id,name,phone,is_admin) VALUES(org_id,'Facebook fixture','0999999320',true) RETURNING id,username INTO actor_id,actor_name;
  ...
  INSERT INTO public.org_staff(organization_id,name,phone,is_admin) VALUES(org_id,'Other fixture','0999999321',false) RETURNING id,username INTO other_actor,other_name;
  INSERT INTO public.auto_accounts(name,flatform_type,staff_id,organization_id,is_delete,facebook_uid,login_status,status)
    VALUES('existing logged-in account','facebook',actor_id,org_id,false,'100000000002222','đã đăng nhập','tạm dừng') RETURNING id INTO enrollment_id;
  SELECT count(*) INTO total_accounts FROM public.auto_accounts WHERE staff_id=actor_id;
  EXECUTE 'SET LOCAL ROLE anon';
  r:=public.aka_agent_facebook_login(actor_id,actor_name,'123456','metadata',jsonb_build_object('accountId',enrollment_id));
  IF r IS DISTINCT FROM '{"uid":"100000000002222","revision":0,"hasPassword":false,"hasTwoFactor":false,"hasCookie":false}'::jsonb THEN RAISE EXCEPTION 'manual metadata invalid'; END IF;
  BEGIN
    PERFORM public.aka_agent_facebook_login(other_actor,other_name,'123456','metadata',jsonb_build_object('accountId',enrollment_id)); RAISE EXCEPTION 'other staff metadata accepted';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM<>'facebook_not_found' THEN RAISE; END IF; END;
  BEGIN
    PERFORM public.aka_agent_facebook_login(actor_id,actor_name,'123456','login',jsonb_build_object('accountId',enrollment_id,'revision',0,'secret',payload->'secret')); RAISE EXCEPTION 'unverified cookie accepted';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM<>'facebook_invalid_input' THEN RAISE; END IF; END;
  EXECUTE 'RESET ROLE';
  IF EXISTS(SELECT 1 FROM public.auto_facebook_login_sessions WHERE auto_facebook_login_sessions.account_id=enrollment_id)
     OR (SELECT facebook_login_managed FROM public.auto_accounts WHERE id=enrollment_id) THEN RAISE EXCEPTION 'failed login enrolled account'; END IF;
  EXECUTE 'SET LOCAL ROLE anon';
  -- Existing session UID 2222 can explicitly become 1111; UID 1111 already exists on another account.
  r:=public.aka_agent_facebook_login(actor_id,actor_name,'123456','login',jsonb_build_object('accountId',enrollment_id,'revision',0,'secret',
    (payload->'secret')||jsonb_build_object('cookies',(payload->'secret'->'cookies')||'[{"name":"xs","value":"fixture-session"}]'::jsonb)));
  IF (r->>'revision')::bigint<>1 THEN RAISE EXCEPTION 'enrollment revision invalid'; END IF;
  BEGIN
    PERFORM public.aka_agent_facebook_login(actor_id,actor_name,'123456','login',jsonb_build_object('accountId',enrollment_id,'revision',0,'secret',payload->'secret')); RAISE EXCEPTION 'stale login accepted';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM<>'facebook_conflict' THEN RAISE; END IF; END;
  r:=public.aka_agent_facebook_login(actor_id,actor_name,'123456','get',jsonb_build_object('accountId',enrollment_id));
  IF r->'secret'->>'password'<>'fixture-password' OR r->'secret'->>'uid'<>'100000000001111' THEN RAISE EXCEPTION 'enrollment lost credentials'; END IF;
  -- Relogin on an already-managed, logged-in account updates the same row/secret.
  PERFORM public.aka_agent_facebook_login(actor_id,actor_name,'123456','login',jsonb_build_object('accountId',enrollment_id,'revision',1,'secret',r->'secret'));
  r:=public.aka_agent_facebook_login(actor_id,actor_name,'123456','metadata',jsonb_build_object('accountId',enrollment_id));
  IF r->>'revision'<>'2' OR r ? 'secret' OR r ? 'password' OR r ? 'cookies' OR r->>'hasTwoFactor'<>'true' THEN RAISE EXCEPTION 'relogin metadata invalid'; END IF;
  EXECUTE 'RESET ROLE';
  IF (SELECT count(*) FROM public.auto_accounts WHERE staff_id=actor_id)<>total_accounts THEN RAISE EXCEPTION 'enrollment created account'; END IF;
  IF NOT (SELECT facebook_login_managed AND facebook_uid='100000000001111' AND login_status='đã đăng nhập' AND status='tạm dừng'
    FROM public.auto_accounts WHERE id=enrollment_id) THEN RAISE EXCEPTION 'enrollment changed unrelated account state'; END IF;
  EXECUTE 'SET LOCAL ROLE anon';
...
END $smoke$;
SELECT 'facebook_v320_smoke_passed' AS result;
ROLLBACK;
