-- Rollback smoke test for migration_v173_automation_target_data_schedule.sql.
--
-- Run only after migration v173 has been applied. The harness deliberately
...
BEGIN
  ...
  IF v_function_count <> 1
    ...
    RAISE EXCEPTION
      'v173_smoke: save RPC overload/signature mismatch (count %, args %, defaults %)',
      v_function_count,
      v_argument_count,
      v_default_count;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'v173_smoke: canonical schedule columns are missing';
  END IF;

  IF to_regprocedure(
    'public.claim_auto_automation_details(bigint,bigint,text,integer,text,text)'
  ) IS NULL THEN
    RAISE EXCEPTION 'v173_smoke: claim RPC is missing';
  END IF;

  IF to_regprocedure(
    'public.materialize_auto_automation_detail(bigint,bigint,bigint,text,jsonb,text,text)'
  ) IS NULL THEN
    RAISE EXCEPTION 'v173_smoke: materialize RPC is missing';
  END IF;
END;
$preflight$;
...
CREATE TEMP TABLE v173_smoke_context (
...
INSERT INTO pg_temp.v173_smoke_context (
  marker,
  staff_id,
  organization_id,
  account_id,
...
BEGIN
  SELECT *
  INTO v_context
  FROM pg_temp.v173_smoke_context;

  IF NOT FOUND THEN
    RAISE EXCEPTION
      'v173_smoke: no isolated tenant with a bidirectional phone action is available';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'v173_smoke: marker rows already exist before the test';
  END IF;
  ...
  IF NOT pg_try_advisory_xact_lock(hashtextextended(
    ...
    RAISE EXCEPTION 'v173_smoke: selected tenant automation graph is busy';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'v173_smoke: selected tenant gained an automation before isolation lock';
  END IF;
  ...
  UPDATE pg_temp.v173_smoke_context
...
CREATE FUNCTION pg_temp.v173_smoke_make_target(
  p_label text,
  p_status text
)
RETURNS bigint
...
CREATE FUNCTION pg_temp.v173_smoke_save_rule(
  p_label text,
...
BEGIN
  ...
  IF p_old_client THEN
    -- Deliberately use only the v171 argument set. The canonical v173 suffix
  ...
  IF v_rule_id IS NULL THEN
    RAISE EXCEPTION 'v173_smoke: save RPC returned no rule id for %', p_label;
  END IF;
...
CREATE FUNCTION pg_temp.v173_smoke_emit(
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'v173_smoke: rule % is not active for emit', p_automation_id;
  END IF;
  ...
  IF v_execution_id IS NULL THEN
    ...
    RAISE EXCEPTION
      'v173_smoke: enqueue produced no execution for % (failure: %)',
      p_label,
...
CREATE FUNCTION pg_temp.v173_smoke_claim_one(
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'v173_smoke safety: refusing to claim a tenant containing a real active automation';
  END IF;
  ...
  IF v_eligible_count <> 1 OR NOT EXISTS (
    ...
    RAISE EXCEPTION
      'v173_smoke safety: expected only execution %, found % eligible rows',
      p_execution_id,
      v_eligible_count;
  END IF;
  ...
  IF v_claimed_count <> 1 OR v_matching_count <> 1 THEN
    RAISE EXCEPTION
      'v173_smoke: claim expected execution %, got % rows/% matches',
      p_execution_id,
      v_claimed_count,
      v_matching_count;
  END IF;
...
CREATE FUNCTION pg_temp.v173_smoke_materialize(
  p_execution_id bigint,
  p_worker_id text
)
RETURNS jsonb
...
DECLARE
  ...
  v_local_day date;
BEGIN
  v_target_campaign_id := pg_temp.v173_smoke_make_target(
    'schedule_math',
    'chờ xử lý'
  );
  ...
  LOOP
    v_rule_id := pg_temp.v173_smoke_save_rule(
      v_case.label,
    ...
    v_execution_id := pg_temp.v173_smoke_emit(
      v_rule_id,
      v_case.label,
      v_event_at
    );
    ...
    IF v_execution.scheduled_at IS DISTINCT FROM v_expected_at
      ...
      RAISE EXCEPTION 'v173_smoke: schedule math failed for %', v_case.label;
    END IF;
  ...
  IF v_execution.scheduled_at IS DISTINCT FROM v_event_at
    ...
    RAISE EXCEPTION 'v173_smoke: immediate schedule calculation failed';
  END IF;
  ...
  LOOP
    ...
    IF v_execution.scheduled_at IS DISTINCT FROM v_expected_at
      ...
      RAISE EXCEPTION 'v173_smoke: daily_time calculation failed for %', v_case.label;
    END IF;
...
BEGIN
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'v173_smoke: trigger/reconcile idempotency created % executions', v_count;
  END IF;

  -- Editing the rule after enqueue must not change the execution snapshot.
  PERFORM pg_temp.v173_smoke_save_rule(
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'v173_smoke: queued execution snapshot changed after rule edit';
  END IF;

  PERFORM pg_temp.v173_smoke_claim_one(
    v_execution_id,
    'v173-smoke-future-worker'
  );
  v_result := pg_temp.v173_smoke_materialize(
    v_execution_id,
    'v173-smoke-future-worker'
  );

  IF v_result ->> 'result' <> 'materialized' THEN
    RAISE EXCEPTION 'v173_smoke: future materialize returned %', v_result;
  END IF;
  ...
  IF v_input.schedule IS DISTINCT FROM v_expected_at
    ...
    RAISE EXCEPTION 'v173_smoke: future target input is missing or already due';
  END IF;
  ...
  IF v_campaign.status <> 'chờ xử lý'
    ...
    RAISE EXCEPTION 'v173_smoke: pending B runtime/original schedule mismatch';
  END IF;
  ...
  IF v_result ->> 'result' <> 'already_materialized' OR v_count <> 1 THEN
    RAISE EXCEPTION 'v173_smoke: materialize idempotency failed (% / %)', v_result, v_count;
  END IF;
END;
...
BEGIN
  ...
  LOOP
    ...
    IF v_result ->> 'result' <> 'materialized'
      ...
      RAISE EXCEPTION 'v173_smoke: target state failed for % (% / %)',
        v_case.label,
        v_result,
        v_campaign.status;
    END IF;
    ...
    IF v_case.initial_status = 'hoàn thành'
      ...
      RAISE EXCEPTION 'v173_smoke: completed B was not fully reopened';
    END IF;
  END LOOP;
END;
$target_states$;
...
BEGIN
  ...
  IF v_result ->> 'result' <> 'target_running'
    ...
    OR NOT EXISTS (
      ...
      WHERE id = v_execution_id
        ...
        AND locked_by = 'v173-smoke-running-worker-1'
    ) THEN
    RAISE EXCEPTION 'v173_smoke: running B was not deferred safely (%)', v_result;
  END IF;
  ...
  v_retry := public.retry_auto_automation_detail(
    ...
    p_worker_id => 'v173-smoke-running-worker-1',
  ...
  IF v_retry ->> 'result' <> 'retry_scheduled' THEN
    RAISE EXCEPTION 'v173_smoke: running B retry was not scheduled (%)', v_retry;
  END IF;
  ...
  IF v_result ->> 'result' <> 'materialized'
    ...
    RAISE EXCEPTION 'v173_smoke: running B retry materialization failed (%)', v_result;
  END IF;
END;
$target_running$;
...
BEGIN
  ...
  IF v_result ->> 'result' <> 'failed'
    ...
    RAISE EXCEPTION 'v173_smoke: deleted B handling failed (%)', v_result;
  END IF;
END;
$target_deleted$;
...
BEGIN
  ...
  IF v_rule.delay_days <> 1
    ...
    RAISE EXCEPTION 'v173_smoke: old-client after_delay compatibility failed';
  END IF;
  ...
  IF v_rule.delay_value IS NOT NULL
    ...
    RAISE EXCEPTION 'v173_smoke: legacy fixed_at snapshot failed';
  END IF;

  PERFORM pg_temp.v173_smoke_claim_one(
    v_execution_id,
    'v173-smoke-fixed-worker'
  );
  v_result := pg_temp.v173_smoke_materialize(
    v_execution_id,
    'v173-smoke-fixed-worker'
  ...
  IF v_result ->> 'result' <> 'materialized'
    ...
    RAISE EXCEPTION 'v173_smoke: legacy fixed_at materialization failed (%)', v_result;
  END IF;

  -- A fixed schedule that was valid when saved still preserves v171's
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'v173_smoke: expired fixed_at compatibility failed';
  END IF;
END;
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      RAISE EXCEPTION
        'v173_smoke: invalid delay unexpectedly accepted for %',
        v_case.label;
    EXCEPTION
      WHEN OTHERS THEN
...
DO $post_rollback$
DECLARE
  v_marker constant text := '__codex_v173_rollback_smoke__';
  v_remaining bigint;
BEGIN
  ...
  IF COALESCE(v_remaining, 0) <> 0 THEN
    RAISE EXCEPTION
      'v173_smoke: rollback left % marker rows',
      v_remaining;
  END IF;
END;
$post_rollback$;
...
SELECT jsonb_build_object(
  'test', 'migration_v173_automation_target_data_schedule_smoke',
  'passed', true,
