...
DECLARE
  ...
  prefix text:='v289-smoke-'||txid_current()::text; total bigint;
BEGIN
  ...
  IF a.id IS NULL THEN RAISE EXCEPTION 'v289_smoke:no_eligible_chat_account'; END IF;
  ...
  IF b.id IS NULL THEN RAISE EXCEPTION 'v289_smoke:second_account_required'; END IF;
  ...
  IF (result->>'bound_zalo_account_id')::bigint IS DISTINCT FROM a.id THEN RAISE EXCEPTION 'v289_smoke:create_binding'; END IF;
  IF public.aka_agent_create_data_group_v2(a.staff_id,a.organization_id,prefix,'#123456',prefix,typ,a.id,NULL,NULL) IS DISTINCT FROM result THEN RAISE EXCEPTION 'v289_smoke:create_replay'; END IF;
  BEGIN
    PERFORM public.aka_agent_create_data_group_v2(a.staff_id,a.organization_id,prefix,'#123456',prefix,typ,NULL,NULL,NULL);
    RAISE EXCEPTION 'v289_smoke:missing_create_conflict';
  ...
  IF c IS NULL OR NOT EXISTS(SELECT 1 FROM public.auto_account_contacts WHERE id=c AND account_id=a.id) THEN RAISE EXCEPTION 'v289_smoke:default_upload_account'; END IF;
  IF NOT EXISTS(SELECT 1 FROM public.auto_account_contact_group_member_origins WHERE membership_id=member_id AND source_account_id=a.id AND is_current) THEN RAISE EXCEPTION 'v289_smoke:upload_provenance'; END IF;
  IF public.aka_agent_ingest_data_group(a.staff_id,a.organization_id,prefix||'-ingest',g,'upload',rows,NULL,prefix,'textbox',NULL,NULL,NULL,typ) IS DISTINCT FROM result THEN RAISE EXCEPTION 'v289_smoke:ingest_replay'; END IF;
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_cross_account_rejection';
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_member_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%data_group_bound_source_mismatch%' THEN RAISE; END IF; END;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_rebind_guard';
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_type_guard';
  ...
  IF (result->>'bound_zalo_account_id')::bigint IS DISTINCT FROM a.id THEN RAISE EXCEPTION 'v289_smoke:duplicate_binding'; END IF;
  BEGIN
    UPDATE public.auto_account_contacts SET account_id=b.id WHERE id=c;
    RAISE EXCEPTION 'v289_smoke:missing_contact_identity_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%data_group_bound_source_mismatch%' THEN RAISE; END IF; END;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_move_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%data_group_bound_source_mismatch%' THEN RAISE; END IF; END;
  IF NOT EXISTS(SELECT 1 FROM public.auto_account_contact_group_members WHERE group_id=mixed AND contact_id=other_c AND NOT is_delete) THEN RAISE EXCEPTION 'v289_smoke:move_rollback'; END IF;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_reactivation_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%data_group_bound_source_mismatch%' THEN RAISE; END IF; END;
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_scoped_tag_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%data_group_bound_rule_mismatch%' THEN RAISE; END IF; END;
  IF NOT EXISTS(SELECT 1 FROM public.aka_agent_list_data_groups_v2(a.staff_id,a.organization_id,prefix,NULL,NULL,NULL,0,100,NULL,NULL,false) r WHERE (r->>'id')::bigint=g AND (r->>'bound_zalo_account_id')::bigint=a.id) THEN RAISE EXCEPTION 'v289_smoke:list_group_binding'; END IF;
  ...
  IF facts->>'zalo_name'<>'Tên gốc 286' OR facts->>'display_name'<>'Biệt danh 286' OR facts->>'zalo_friend_status'<>'request_received' THEN RAISE EXCEPTION 'v289_smoke:chat_facts'; END IF;
  ...
  IF total<>1 THEN RAISE EXCEPTION 'v289_smoke:search_status_before_pagination'; END IF;
  FOREACH v_status_code IN ARRAY ARRAY['friend','request_sent','request_received','stranger','unknown','removed'] LOOP
    ...
    IF public.aka_agent_data_group_zalo_facts(c)->>'zalo_friend_status' IS DISTINCT FROM (CASE WHEN v_status_code IN ('friend','request_sent','request_received') THEN v_status_code ELSE 'stranger' END) THEN RAISE EXCEPTION 'v289_smoke:status:%',v_status_code; END IF;
  END LOOP;
  ...
  IF NOT public.aka_agent_data_group_dynamic_values_match(c,'zalo_tag',ARRAY[a.id::text||':'||prefix]) OR NOT public.aka_agent_data_group_dynamic_values_match(c,'akabiz_tag',ARRAY[tag::text]) THEN RAISE EXCEPTION 'v289_smoke:chat_tag_match'; END IF;
  ...
  IF public.aka_agent_data_group_dynamic_values_match(c,'zalo_tag',ARRAY[prefix]) OR public.aka_agent_data_group_dynamic_values_match(c,'akabiz_tag',ARRAY[tag::text]) THEN RAISE EXCEPTION 'v289_smoke:stale_local_tag_resurrection'; END IF;
  ...
  IF (result->>'queued_count')::int<>0 THEN RAISE EXCEPTION 'v289_smoke:historical_enqueue'; END IF;
  ...
  IF NOT EXISTS(SELECT 1 FROM public.auto_account_contact_dynamic_filter_queue WHERE contact_id=c) THEN RAISE EXCEPTION 'v289_smoke:chat_event_not_queued'; END IF;
  SELECT id INTO rule_id FROM public.auto_account_contact_group_dynamic_filter_rules WHERE dynamic_filter_id=f;
  IF NOT public.aka_agent_data_group_dynamic_rule_matches(rule_id,c) OR public.aka_agent_data_group_dynamic_rule_matches(rule_id,other_c) THEN RAISE EXCEPTION 'v289_smoke:bound_dynamic_scope'; END IF;
  ...
  IF public.aka_agent_data_group_dynamic_rule_matches(rule_id,other_c) THEN RAISE EXCEPTION 'v289_smoke:negation_cross_account'; END IF;
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_save_rule_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%data_group_bound_rule_mismatch%' THEN RAISE; END IF; END;
  result:=public.aka_agent_get_data_group_dynamic_filter(a.staff_id,a.organization_id,g,NULL,NULL);
  IF (result->'filter'->>'bound_zalo_account_id')::bigint IS DISTINCT FROM a.id OR EXISTS(SELECT 1 FROM jsonb_array_elements(result->'values') v WHERE v->>'account_id' IS NOT NULL AND (v->>'account_id')::bigint<>a.id) THEN RAISE EXCEPTION 'v289_smoke:catalog_scope'; END IF;
  ...
  IF result->>'bound_zalo_account_id' IS NOT NULL THEN RAISE EXCEPTION 'v289_smoke:unbind'; END IF;
  ...
  IF NOT EXISTS(SELECT 1 FROM public.chat_zalo_conversation_system_tag WHERE chat_zalo_conversation_id=conv AND auto_contact_tag_id=tag)
    OR NOT public.aka_agent_data_group_dynamic_values_match(c,'akabiz_tag',ARRAY[tag::text]) THEN RAISE EXCEPTION 'v289_smoke:desktop_write_not_canonical'; END IF;
  ...
  IF NOT (SELECT akabiz_tag_ids @> ARRAY[tag,tag2] FROM public.auto_account_contacts WHERE id=c) THEN RAISE EXCEPTION 'v289_smoke:chat_add_not_mirrored'; END IF;
  DELETE FROM public.chat_zalo_conversation_system_tag WHERE chat_zalo_conversation_id=conv AND auto_contact_tag_id=tag;
  IF (SELECT tag=ANY(akabiz_tag_ids) FROM public.auto_account_contacts WHERE id=c) THEN RAISE EXCEPTION 'v289_smoke:chat_remove_not_mirrored'; END IF;
  ...
  IF (result->>'count')::integer<>1 OR NOT (SELECT akabiz_tag_ids @> ARRAY[tag,tag2] FROM public.auto_account_contacts WHERE id=c) THEN RAISE EXCEPTION 'v289_smoke:additive_rpc'; END IF;
  IF (public.aka_agent_mutate_contact_tags(a.staff_id,a.organization_id,ARRAY[c],ARRAY[tag],NULL,NULL,'add')->>'count')::integer<>0 THEN RAISE EXCEPTION 'v289_smoke:rpc_replay'; END IF;
  BEGIN
    PERFORM public.aka_agent_mutate_contact_tags(a.staff_id,a.organization_id,ARRAY[c],ARRAY[bad_tag],NULL,NULL,'add');
    RAISE EXCEPTION 'v289_smoke:missing_rpc_tag_scope_guard';
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_legacy_tag_scope_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%contact_tag_scope_invalid%' THEN RAISE; END IF; END;
  IF current_setting('aka_agent.chat_tag_writeback',true)='on' OR current_setting('aka_agent.chat_tag_projection',true)='on' THEN RAISE EXCEPTION 'v289_smoke:sync_context_leaked'; END IF;
  UPDATE public.auto_account_contacts SET akabiz_tag_ids=ARRAY[tag2] WHERE id=c;
  IF EXISTS(SELECT 1 FROM public.chat_zalo_conversation_system_tag WHERE chat_zalo_conversation_id=conv AND auto_contact_tag_id=tag) THEN RAISE EXCEPTION 'v289_smoke:desktop_remove_not_canonical'; END IF;
  ...
  IF (SELECT tag=ANY(akabiz_tag_ids) OR NOT tag2=ANY(akabiz_tag_ids) FROM public.auto_account_contacts WHERE id=c)
    OR EXISTS(SELECT 1 FROM public.chat_zalo_conversation_system_tag WHERE chat_zalo_conversation_id=conv AND auto_contact_tag_id=tag) THEN RAISE EXCEPTION 'v289_smoke:delta_remove'; END IF;
  ...
  IF (result->>'count')::integer<>1 OR NOT (SELECT tag=ANY(akabiz_tag_ids) FROM public.auto_account_contacts WHERE id=other_c) THEN RAISE EXCEPTION 'v289_smoke:local_fallback'; END IF;
  IF (public.aka_agent_mutate_contact_tags(a.staff_id,-1,ARRAY[other_c],ARRAY[tag],NULL,NULL,'add')->>'count')::integer<>0 THEN RAISE EXCEPTION 'v289_smoke:tenant_scope'; END IF;
  ...
  IF (SELECT is_enabled FROM public.auto_account_contact_group_dynamic_filters WHERE group_id=dyn) THEN RAISE EXCEPTION 'v289_smoke:disabled_filter_still_enabled'; END IF;
  BEGIN
    PERFORM public.aka_agent_save_data_group_dynamic_filter(a.staff_id,a.organization_id,dyn,true,rows,NULL,NULL);
    RAISE EXCEPTION 'v289_smoke:reenabled_invalid_rule';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%data_group_bound_rule_mismatch%' THEN RAISE; END IF; END;
  ...
  IF NOT (SELECT is_delete FROM public.auto_account_contacts WHERE id=c) THEN RAISE EXCEPTION 'v289_smoke:chat_event_revived_contact'; END IF;
  IF (public.aka_agent_mutate_contact_tags(a.staff_id,a.organization_id,ARRAY[c],ARRAY[tag],NULL,NULL,'add')->>'count')::integer<>0 THEN RAISE EXCEPTION 'v289_smoke:tag_rpc_touches_deleted_contact'; END IF;
  ...
  IF EXISTS(SELECT 1 FROM public.auto_account_contact_group_members WHERE group_id=dyn AND contact_id=c AND NOT is_delete) THEN RAISE EXCEPTION 'v289_smoke:worker_reentered_deleted_contact'; END IF;
  IF EXISTS(SELECT 1 FROM public.auto_account_contact_dynamic_filter_queue WHERE contact_id=c) THEN RAISE EXCEPTION 'v289_smoke:event_not_processed'; END IF;
  IF has_function_privilege('anon','public.aka_agent_refresh_contact_chat_tags(bigint)','EXECUTE') OR has_function_privilege('service_role','public.aka_agent_sync_contact_chat_tag_delta()','EXECUTE') THEN RAISE EXCEPTION 'v289_smoke:private_helper_exposed'; END IF;

  IF has_function_privilege('anon','public.aka_agent_data_group_zalo_facts(bigint)','EXECUTE') THEN RAISE EXCEPTION 'v289_smoke:helper_exposure'; END IF;
  ...
  BEGIN
    ...
    RAISE EXCEPTION 'v289_smoke:missing_auth_guard';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%automation_auth_required%' THEN RAISE; END IF; END;
  BEGIN
    PERFORM public.aka_agent_mutate_contact_tags(a.staff_id,a.organization_id,ARRAY[c],ARRAY[tag],NULL,NULL,'add');
    RAISE EXCEPTION 'v289_smoke:missing_tag_auth';
  EXCEPTION WHEN OTHERS THEN IF SQLERRM NOT LIKE '%automation_auth_required%' THEN RAISE; END IF; END;
  PERFORM set_config('v289.smoke_result','passed: canonical tags, legacy deltas, additive RPC, scope/auth, disable/re-enable, tombstones and worker',true);
  RAISE NOTICE 'v289 smoke passed: bindings, ingest, guards, Chat facts/tags/status/events, pagination, ACL/auth';
END;
$smoke$;
SELECT current_setting('v289.smoke_result') AS smoke_result;
ROLLBACK;
