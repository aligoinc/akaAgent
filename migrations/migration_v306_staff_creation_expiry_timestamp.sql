-- v306: use one DB timestamp for created_at and the Vietnam expiry date.
-- Exact live v305 body captured before drafting; no signature/API metadata change.
BEGIN;
...
DO $preflight$ DECLARE r record; BEGIN
 ...
 IF NOT FOUND OR r.checksum NOT IN ('b0fb02514a2b501b1e6d94d1122d330d','45158ff8555a5a12501949371d08b658') OR r.attrs IS DISTINCT FROM '{"owner": "postgres", "prosecdef": true, "provolatile": "v", "proconfig": ["search_path=pg_catalog, public", "lock_timeout=3s", "statement_timeout=12s"], "proacl": "{postgres=X/postgres,anon=X/postgres,authenticated=X/postgres,service_role=X/postgres}"}'::jsonb THEN RAISE EXCEPTION 'v306 staff management live definition/attribute drift'; END IF;
END; $preflight$;
...
-- Body-only change: no explicit PostgREST schema reload.
COMMIT;
