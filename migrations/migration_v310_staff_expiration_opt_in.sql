-- v310: staff expiration is opt-in; false means organization/product expiration.
-- Source: exact linked cgjbsmqtfhqvttudyjzq definitions captured 2026-09-23.
-- Existing organizations intentionally receive false, NOT the inverse of the old column.
-- Keep the old column for schema compatibility; only use_staff_expiration controls policy.
-- Legacy RPC JSON useOrganizationExpiration remains the inverse of the canonical flag.
BEGIN;
...
DO $preflight$ DECLARE r record; BEGIN
  ...
  IF NOT FOUND OR r.checksum NOT IN ('878cba86a2426dae09bc9f69a163680e','a294e60011edd8d42e90ac2f74ce744c')
    ...
    RAISE EXCEPTION 'v310 definition/attribute drift: public.aka_agent_staff_time_allowed(bigint)';
  END IF;
  ...
  IF NOT FOUND OR r.checksum NOT IN ('632942ca116ca830d4a7f5fff72e871f','ca59bfbfa79c7e40ec50788e77b9d022')
    ...
    RAISE EXCEPTION 'v310 definition/attribute drift: public.aka_agent_staff_access(bigint,text,text)';
  END IF;
  ...
  IF NOT FOUND OR r.checksum NOT IN ('e4d06c369e7c05e29c69ce089e549da6','4964edb52ffc866f610036212d14fb08')
    ...
    RAISE EXCEPTION 'v310 definition/attribute drift: public.aka_agent_staff_management_row(bigint)';
  END IF;
  ...
  IF NOT FOUND OR r.checksum NOT IN ('fe73d8c5812d395aa2d3a1d448418ef0','689c389e3380c75497c720e7e6c5dafb')
    ...
    RAISE EXCEPTION 'v310 definition/attribute drift: public.aka_agent_staff_management(bigint,text,text,text,jsonb)';
  END IF;
END; $preflight$;

-- Match existing runtime table lock order. No data rewrite or per-organization UPDATE.
LOCK TABLE public.org_staff, public.org_organization IN ACCESS EXCLUSIVE MODE;
DO $column$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid='public.org_organization'::regclass
    AND attname='use_staff_expiration' AND NOT attisdropped) THEN
    ALTER TABLE public.org_organization ADD COLUMN use_staff_expiration boolean NOT NULL DEFAULT false;
  ELSIF NOT EXISTS (SELECT 1 FROM pg_attribute a JOIN pg_attrdef d ON d.adrelid=a.attrelid AND d.adnum=a.attnum
    WHERE a.attrelid='public.org_organization'::regclass AND a.attname='use_staff_expiration'
      ...
      AND a.attgenerated='' AND pg_get_expr(d.adbin,d.adrelid)='false') THEN
    RAISE EXCEPTION 'v310 use_staff_expiration column drift';
  END IF;
END; $column$;
COMMENT ON COLUMN public.org_organization.use_staff_expiration IS
  'Canonical expiry mode: false (default) uses organization products; true additionally checks staff expiry, NULL staff expiry falls back to products.';
COMMENT ON COLUMN public.org_organization.use_organization_expiration IS
  'Deprecated compatibility column; no longer controls expiry. Configure use_staff_expiration instead. Legacy RPC JSON is derived from the new flag.';
...
AS $function$
  SELECT coalesce((SELECT s.is_active AND s.deleted_at IS NULL AND
    (NOT o.use_staff_expiration OR s.expiration_date IS NULL OR
...
BEGIN
  ...
  RETURN jsonb_build_object('isAdmin',s.is_admin IS TRUE AND s.is_active AND s.deleted_at IS NULL,
    ...
    'expirationDate',s.expiration_date,'useStaffExpiration',o.use_staff_expiration,'useOrganizationExpiration',NOT o.use_staff_expiration);
END;
$function$;
...
AS $function$
  SELECT jsonb_build_object('id',s.id,'name',s.name,'phone',s.phone,'username',s.username,
    ...
    'expirySource',CASE WHEN NOT o.use_staff_expiration OR s.expiration_date IS NULL THEN 'organization' ELSE 'staff' END,
  ...
  CROSS JOIN LATERAL (SELECT CASE WHEN NOT o.use_staff_expiration OR s.expiration_date IS NULL THEN p.expires
...
BEGIN
  ...
  IF p_action='list' THEN
    ...
    result:=result||jsonb_build_object('organization',jsonb_build_object('id',org.id,'name',org.name,'maxStaff',org.max_staff,
      'staffCount',total,'staffDurationDays',coalesce(org.staff_duration_days,365),'useStaffExpiration',org.use_staff_expiration,'useOrganizationExpiration',NOT org.use_staff_expiration,
...
-- New API column requires a PostgREST schema refresh.
