-- Add sorted campaign input paging using the captured production v219/v221/v230 definitions.
-- Existing overloads and runtime ordering remain intact. No data/column/trigger changes.
BEGIN;
...
BEGIN
  v_oid := to_regprocedure('public.aka_agent_list_campaign_input_data_page(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer)');
  IF v_oid IS NULL OR md5(pg_get_functiondef(v_oid)) <> 'd0d1a242529d6fc83a1cdf557ee0cc63' THEN
    RAISE EXCEPTION 'v313_source_definition_changed: aka_agent_list_campaign_input_data_page(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer)';
  END IF;
  v_oid := to_regprocedure('public.aka_agent_list_campaign_input_data_page(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text,text)');
  IF v_oid IS NULL OR md5(pg_get_functiondef(v_oid)) <> '2ac65746a79de69ba99911b3301e1340' THEN
    RAISE EXCEPTION 'v313_source_definition_changed: aka_agent_list_campaign_input_data_page(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text,text)';
  END IF;
  ...
  IF v_oid IS NOT NULL AND md5(pg_get_functiondef(v_oid)) <> '26f1c96183a8d0e92ac76575e8c1df96' THEN
    RAISE EXCEPTION 'v313_unexpected_existing_target: aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text)';
  END IF;
  ...
  IF v_oid IS NOT NULL AND md5(pg_get_functiondef(v_oid)) <> '440e2c6163518d4c0563c275d33e8474' THEN
    RAISE EXCEPTION 'v313_unexpected_existing_target: aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text,text,text)';
  END IF;
END;
$preflight$;

CREATE OR REPLACE FUNCTION public.aka_agent_list_campaign_input_data_page_v2(p_staff_id bigint, p_organization_id bigint, p_campaign_id bigint, p_search text, p_status text, p_date_from timestamp with time zone, p_date_to timestamp with time zone, p_origin_filter text, p_offset integer, p_limit integer, p_sort text)
...
$function$
;
ALTER FUNCTION public.aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text) TO service_role;

CREATE OR REPLACE FUNCTION public.aka_agent_list_campaign_input_data_page_v2(p_staff_id bigint, p_organization_id bigint, p_campaign_id bigint, p_search text, p_status text, p_date_from timestamp with time zone, p_date_to timestamp with time zone, p_origin_filter text, p_offset integer, p_limit integer, p_auth_username text, p_auth_password text, p_sort text)
 RETURNS TABLE(input_data jsonb, origins jsonb, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
BEGIN
  ...
  BEGIN
    ...
    FROM public.aka_agent_list_campaign_input_data_page_v2(
      ...
      p_date_from, p_date_to, p_origin_filter, p_offset, p_limit, p_sort
    );
...
$function$
;
ALTER FUNCTION public.aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text,text,text) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text,text,text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.aka_agent_list_campaign_input_data_page_v2(bigint,bigint,bigint,text,text,timestamp with time zone,timestamp with time zone,text,integer,integer,text,text,text) TO anon, authenticated, service_role;

-- New RPC metadata must be visible before the updated client calls it.
