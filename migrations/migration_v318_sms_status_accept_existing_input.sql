-- v318: accept SMS reports despite source deletion/account reassignment.
-- API authentication, existing detail ownership, normal status ordering,
-- once-only counters and new-data fetch filters remain unchanged.
-- Derived from the captured production v317 definition; no device/source checks.
BEGIN;
...
BEGIN
  ...
  IF v_oid IS NULL OR md5(pg_get_functiondef(v_oid)) NOT IN
    ('2efe388f627258da23e79842b271a16f', '6ed7c94a13aa8d1644fa9bbb401f10b2') THEN
    RAISE EXCEPTION 'v318_definition_changed';
  END IF;
  IF md5(pg_get_functiondef(to_regprocedure('public.aka_agent_enqueue_group_only_automations()')))
      IS DISTINCT FROM 'c6611cee9b1c40772bdb9e41234049de'
    OR md5(pg_get_functiondef(to_regprocedure('public.aka_agent_list_sms_due_input_data(bigint,bigint[],text,integer,integer)')))
      IS DISTINCT FROM '70ffadf2302e9601f68d1af0dab41469' THEN
    RAISE EXCEPTION 'v318_dependency_changed';
  END IF;
END;
$preflight$;
