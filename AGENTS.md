...
- `auto_status` / `auto_status_usage` — catalog trạng thái dùng cho UI/options theo `flatform_type` + `component_type` (`account`, `account_login`, `campaign`, `campaign_input`, `campaign_input_data`, `campaign_detail`); hiện chỉ là lookup, chưa thay các cột text runtime như `auto_accounts.status`/`login_status` hoặc campaign status bằng FK.
