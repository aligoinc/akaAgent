...
`facebook_message_friend` là message-only tới bạn bè/contact, luôn chạy `enableMessage=true` + `enableAddFriend=false` và count `fb_message_friend`. `facebook_message_uid` ([migration_v34_message_uid_campaign.sql](migrations/migration_v34_message_uid_campaign.sql)) dùng workflow clone riêng cho UID, count nhắn tin là `fb_message_stranger`, kết bạn là `fb_add_friend`.
