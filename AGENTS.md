...
`facebook_newsfeed_interaction` ([migration_v73_facebook_newsfeed_interaction.sql](migrations/migration_v73_facebook_newsfeed_interaction.sql) → [migration_v76_fix_newsfeed_branch_dataflow.sql](migrations/migration_v76_fix_newsfeed_branch_dataflow.sql)) là no-target campaign chạy hidden/offscreen để lướt newsfeed; scheduler inject `newsfeed*` vars + `allowNewsfeedLike/allowNewsfeedComment`, log detail ngay sau `fb_newsfeed_like_post`/`fb_newsfeed_comment_submit` với action codes `fb_like_post`/`fb_comment`. Partial quota (action nào còn quota thì vẫn chạy) và pause-abort workflow chỉ áp dụng riêng action id này; daily newsfeed không dùng continue-next-day.
...
- **Newsfeed interaction**: `fb_newsfeed_*` blocks should skip unreadable/non-actionable posts instead of failing the whole run; only `liked === true` / `commented === true` action outputs should create `auto_campaign_details` rows. Partial quota and pause abort must stay gated to `facebook_newsfeed_interaction`.
