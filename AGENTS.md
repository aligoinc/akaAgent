...
`WebviewRegistry` maps `accountId -> webContentsId`. Scheduler uses `isRegistered()` to ensure the account has mounted a browser tab at least once; accountPoller uses `listRegistered()` to skip dead tabs. Runtime uses `accountId` and partition `persist:account_${accountId}`; browser profiles from the previous partition prefix are not reused. Account proxy is prepared by `ProxyRuntimeService` on this same partition before visible webview/background page loads; proxy changes close old connections, reload visible webviews when mounted, and destroy hidden background pages so the next run starts clean.
...
- `auto_proxies` — kho proxy staff-scoped dùng chung cho nhiều account; hỗ trợ `http`/`https`/`socks5`, credential lưu DB và mask ở UI. Không cho xoá proxy đang được account dùng; không cho đổi proxy account hoặc sửa proxy đang dùng khi account liên quan có `status='đang chạy'`.
