...
`extraSettings.rewriteSourceContentWithAI/sourceContentAiPrompt` áp dụng riêng phần nội dung copy từ nguồn cho `facebook_timeline_post`/`facebook_page_post`: workflow chạy sau `fb_scrape_post`, gọi `https://api.akaapp.vn/api/AI/chat` với `question` (thay `[content]`, hoặc tự thêm `Nội dung: ...` nếu thiếu placeholder) và `source='aka_agent'`; nếu AI lỗi thì log cảnh báo tiếng Việt và dùng nội dung copy gốc, không làm fail campaign.
