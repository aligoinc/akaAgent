...
`facebook_newsfeed_interaction` ([migration_v73_facebook_newsfeed_interaction.sql](migrations/migration_v73_facebook_newsfeed_interaction.sql) → [migration_v77_newsfeed_reuse_enable_action_flags.sql](migrations/migration_v77_newsfeed_reuse_enable_action_flags.sql)) là no-target campaign chạy hidden/offscreen để lướt newsfeed; `enablePostLike`/`enableComment` là action toggles, còn `newsfeed*Kind` chỉ là điều kiện AI. Scheduler inject `newsfeed*` vars + `allowNewsfeedLike/allowNewsfeedComment`, log detail ngay sau `fb_newsfeed_like_post`/`fb_newsfeed_comment_submit` với action codes `fb_like_post`/`fb_comment`. Partial quota (action nào còn quota thì vẫn chạy) và pause-abort workflow chỉ áp dụng riêng action id này; daily newsfeed không dùng continue-next-day.
