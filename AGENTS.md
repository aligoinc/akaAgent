...
- `auto_campaigns` — campaign config (action_id, account_id, schedule, original_schedule, daily_stop_time, content, extra_settings). `schedule` là next runtime schedule scheduler có thể tự đổi; `original_schedule` là lịch/giờ gốc do user tạo/sửa, dùng làm anchor giờ cho weekly/monthly khi runtime schedule đã bị automation đẩy tới. Status runtime hợp lệ: `'chờ xử lý' | 'đang chạy' | 'tạm dừng' | 'hoàn thành'` (`'lỗi'` không còn là campaign status hợp lệ; lỗi action-level nằm ở `auto_campaign_details`). `daily_stop_time` là giờ dừng trong ngày (`time`, Asia/Ho_Chi_Minh); `getPendingCampaigns()` bỏ qua campaign sau giờ này.
...
**Schedule maintenance**: sau scoped recovery lúc login, `maintainCampaignSchedules()` ([campaignRepository.ts](src/main/data/repositories/campaignRepository.ts)) dọn campaign stale theo ngày `Asia/Ho_Chi_Minh`; day-change watcher trong [handlers.ts](src/main/ipc/handlers.ts) gọi lại khi app treo qua đêm. Weekly/monthly dùng giờ từ `original_schedule ?? schedule` để tính lịch mới; automation chỉ update runtime `schedule`, còn user tạo/sửa campaign mới update cả `schedule` và `original_schedule`. Weekly/monthly `refreshData=true` reset `auto_campaign_input_data` rồi bật lại `chờ xử lý`; completed no-refresh chỉ update `schedule`.
