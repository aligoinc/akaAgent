...
- **FB post image upload**: `page.dropFile()` must avoid viewport coordinates in hidden/offscreen pages; `fileCount` only means files were injected, not that FB rendered an attachment preview ([pageController.ts](src/main/v2/runtime/pageController.ts)).
