...
`facebook_find_data_search` ([migration_v84_find_data_search_ui_action.sql](migrations/migration_v84_find_data_search_ui_action.sql) → [migration_v85_find_data_search_runtime.sql](migrations/migration_v85_find_data_search_runtime.sql)) searches Facebook by keyword with a DB-seeded workflow. It can scan search posts/comments for phones, Zalo group links, UIDs, and post links, and can scan search groups for Facebook group URLs. `buildVariablesV2()` passes post filters, group filters (`minSearchGroupMembers`, `minSearchGroupPostsPerDay`), and search-specific counts; `logMilestonesV2()` writes one `"Tìm data"` detail row with `searchKeyword`, normal find-data fields, `facebookGroups`, `sourceCounts`, and group target campaign IDs. Found Facebook groups fan out to `facebook_group_post` / `facebook_comment_seeding` through `findFacebookGroupPostTargetCampaignIds` and `findFacebookGroupCommentTargetCampaignIds`.
