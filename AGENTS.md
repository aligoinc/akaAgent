...
`facebook_find_data_search` ([migration_v84_find_data_search_ui_action.sql](migrations/migration_v84_find_data_search_ui_action.sql)) is currently UI/settings only: the campaign action is seeded with `workflow_id=NULL` and no workflow/runtime blocks. Its settings support search keywords, post/comment search filters, Facebook group filters (`minSearchGroupMembers`, `minSearchGroupPostsPerDay`), find-data goal priority including `facebook_group`, and group fan-out config to `facebook_group_post` / `facebook_comment_seeding` via `findFacebookGroupPostTargetCampaignIds` and `findFacebookGroupCommentTargetCampaignIds`.
