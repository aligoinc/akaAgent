-- Add optional image support to the existing Facebook comment block.
-- Images are passed per loop item from vars.commentIterations or via
-- vars.commentImageBatches[vars.loopIndex] for workflows that prepare their own iterations.

UPDATE public.auto_blocks
SET
  description = 'Comment vào bài thứ N, hỗ trợ nội dung và ảnh comment (fallback vars.loopItem khi chạy trong loop)',
  config_schema = '[
    ...
    {"name":"images","type":"json","label":"Ảnh comment (array path, để rỗng = lấy từ loopItem/vars)"}
  ]'::jsonb,
  output_schema = '[
...
const images = rawImages.map(x => String(x || '').trim()).filter(Boolean)
