...
const electronAPI = {
  ...
  loginRemembered: (username: string, options?: Partial<LoginPreferences>): Promise<import('../shared/types').AuthLoginResult> =>
    ipcRenderer.invoke(IPC_EVENTS.AUTH_LOGIN_REMEMBERED, username, options),

  acceptPolicyAndLogin: (): Promise<import('../shared/types').AuthLoginResult> =>
    ipcRenderer.invoke(IPC_EVENTS.AUTH_ACCEPT_POLICY_AND_LOGIN),

  cancelPendingLogin: (): Promise<void> => ipcRenderer.invoke(IPC_EVENTS.AUTH_CANCEL_PENDING_LOGIN),
  ...
  recoverDeviceCredentials: (): Promise<AuthLocalState> =>
  ...
  resetDeviceLockByUsername: (username: string): Promise<DeviceLockResetResult> =>
    ipcRenderer.invoke(IPC_EVENTS.AUTH_RESET_DEVICE_LOCK_BY_USERNAME, username),

  changePassword: (oldPassword: string, newPassword: string): Promise<{ success: boolean; loginState: AuthLocalState }> =>
  ...
  // Campaigns
  listCampaignSummaries: (): Promise<CampaignListItem[]> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_LIST_CAMPAIGN_SUMMARIES),

  getCampaignConfig: (id: number): Promise<CampaignConfig | null> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_GET_CAMPAIGN_CONFIG, id),

  getCampaignLog: (id: number): Promise<CampaignLogSnapshot | null> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_GET_CAMPAIGN_LOG, id),

  createCampaign: (data: Partial<Campaign>): Promise<Campaign> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_CREATE_CAMPAIGN, data),

  updateCampaign: (id: number, updates: CampaignUpdate): Promise<Campaign> =>
  ...
  createContentTemplate: (data: CreateContentTemplateInput): Promise<ContentTemplate> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_CREATE_CONTENT_TEMPLATE, data),

  updateContentTemplate: (id: number, updates: UpdateContentTemplateInput): Promise<ContentTemplate> =>
  ...
  listContentTemplateGroups: (): Promise<ContentTemplateGroup[]> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_LIST_CONTENT_TEMPLATE_GROUPS),

  createContentTemplateGroup: (data: CreateContentTemplateGroupInput): Promise<ContentTemplateGroup> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_CREATE_CONTENT_TEMPLATE_GROUP, data),

  updateContentTemplateGroup: (id: number, updates: UpdateContentTemplateGroupInput): Promise<ContentTemplateGroup> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_UPDATE_CONTENT_TEMPLATE_GROUP, id, updates),

  deleteContentTemplateGroup: (id: number): Promise<void> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_DELETE_CONTENT_TEMPLATE_GROUP, id),

  listContentTemplateContentTypes: (): Promise<ContentTemplateContentType[]> =>
    ipcRenderer.invoke(IPC_EVENTS.DB_LIST_CONTENT_TEMPLATE_CONTENT_TYPES),
  ...
  downloadMediaFiles: (ids: number[]): Promise<MediaDownloadResult> =>
    ipcRenderer.invoke(IPC_EVENTS.MEDIA_FILES_DOWNLOAD, ids),
  ...
  listMediaGroups: (): Promise<{ groups: MediaGroup[]; memberships: MediaGroupMembership[] }> =>
  ...
  onCampaignStatusUpdated: (callback: (campaign: CampaignSummaryRefreshSignal) => void): () => void => {
    const handler = (_event: Electron.IpcRendererEvent, campaign: CampaignSummaryRefreshSignal) => callback(campaign)
  ...
  loadPageInboxCustomers: (accountId: number, pageUid: string, pageName?: string, options?: PageInboxScanOptions): Promise<ContactLoadResult> =>
    ipcRenderer.invoke(IPC_EVENTS.CONTACTS_LOAD_PAGE_INBOX_CUSTOMERS, accountId, pageUid, pageName, options),

  getPageInboxScanInfo: (accountId: number, pageUid: string): Promise<PageInboxScanInfo> =>
    ipcRenderer.invoke(IPC_EVENTS.CONTACTS_GET_PAGE_INBOX_SCAN_INFO, accountId, pageUid),
  ...
  listContactDatasets: (query: ContactDatasetListQuery): Promise<AutoAccountContactDataset[]> =>
    ipcRenderer.invoke(IPC_EVENTS.CONTACT_DATASETS_LIST, query),

  saveUploadDataset: (request: SaveUploadDatasetRequest): Promise<SaveUploadDatasetResult> =>
    ipcRenderer.invoke(IPC_EVENTS.CONTACT_DATASETS_SAVE_UPLOAD, request),
  ...
  listDataTypeCategoryItems: (): Promise<DataTypeCategoryItem[]> =>
    ipcRenderer.invoke(IPC_EVENTS.DATA_TYPE_CATEGORIES_LIST),
  ...
  getDataGroupAccountOptions: (query: DataGroupAccountOptionsQuery): Promise<DataGroupAccountOption[]> =>
    ipcRenderer.invoke(IPC_EVENTS.DATA_GROUPS_ACCOUNT_OPTIONS, query),
  ...
  previewDataGroupCampaignTargets: (
    request: DataGroupCampaignTargetPreviewRequest
  ): Promise<DataGroupCampaignTargetPreview[]> =>
    ipcRenderer.invoke(IPC_EVENTS.CAMPAIGN_DATA_GROUP_TARGETS_PREVIEW, request),
  ...
  listZaloFriendBlocklistPage: (accountId: number, query: ZaloFriendBlocklistPageQuery): Promise<ZaloFriendBlocklistPage> =>
    ipcRenderer.invoke(IPC_EVENTS.ZALO_FRIEND_BLOCKLISTS_LIST_PAGE, accountId, query),
  ...
  addFriendsToZaloFriendBlocklist: (groupId: number, contactIds: number[]): Promise<ZaloFriendBlocklistMutationResult> =>
    ipcRenderer.invoke(IPC_EVENTS.ZALO_FRIEND_BLOCKLISTS_ADD_FRIENDS, groupId, contactIds),

  removeFriendsFromZaloFriendBlocklist: (groupId: number, contactIds: number[]): Promise<ZaloFriendBlocklistMutationResult> =>
