...
export function canUseCampaignAction(
  ...
  const normalized = normalizeEntitlements(entitlements)
  if (action.id === SMS_SEND_ACTION_ID || action.id === VOICE_CALL_ACTION_ID || action.flatformType === 'sms') return normalized.sms
...
export function getCampaignActionFeature(
  ...
  if (actionId === SMS_SEND_ACTION_ID || actionId === VOICE_CALL_ACTION_ID || flatformType === 'sms') return 'sms'
...
export function getAccountActionFeature(
  ...
  if (normalizedPlatform === 'sms' || normalizedActionCode === SMS_SEND_ACTION_ID || normalizedActionCode === VOICE_CALL_ACTION_ID) return 'sms'
