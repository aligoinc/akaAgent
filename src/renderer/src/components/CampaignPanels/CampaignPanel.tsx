...
export default function CampaignPanel({ isActive, filterAccountId, accountInfoOpenRequest, onClearFilter, onNavigateToBrowser, onOpenGeneralSettings, onOpenContentTemplates, onAskAssistant, dataGroupCampaignRequest, onDataGroupCampaignRequestHandled }: CampaignPanelProps) {
  ...
  const isSelectedZaloInputCampaign = selectedCampaignSummary?.actionId.startsWith('zalo_') === true
                    ...
                    <table className="campaign-grid" style={{ fontSize: 12 }}>
                      <thead>
                        <tr>
                          ...
                          {isSelectedZaloInputCampaign && (['info1', 'info2', 'info3', 'info4', 'info5'] as const).map((field, index) => (
                      ...
                      <tbody>
                        {filteredCampaignInputData.map(d => (
                          <tr key={d.id} className={selectedInputDataIds.has(d.id) ? 'input-data-selected-row' : ''}>
                            ...
                            {isSelectedZaloInputCampaign && (['info1', 'info2', 'info3', 'info4', 'info5'] as const).map(field => (
