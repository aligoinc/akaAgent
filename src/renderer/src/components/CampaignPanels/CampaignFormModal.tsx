  ...
  const buildCampaignSaveBundleItems = (detailRows: Partial<CampaignInputData>[] = details): CampaignSaveBundleItem[] => {
    ...
    return formData.accountIds.map((accountId, index) => {
      ...
      const advancedContentItemsForSave = canSaveAdvancedContent
        ? normalizedAdvancedContentItems.map(item => isSmsCampaign
      ...
      for (const row of rows) {
        ...
        if (isPhoneOrUidInputCampaign) {
          const normalizedPhone =
            normalizeVietnamMobilePhone(phone) ||
            normalizeVietnamMobilePhone(row[1]) ||
            normalizeVietnamMobilePhone(name)
          if (!normalizedPhone) continue
          importedRows.push({
            ...baseRow,
            name: normalizeVietnamMobilePhone(name) === normalizedPhone ? '' : name,
            uid: '',
                    ...
                    <>
                      ...
                      {isAdvancedContentMode ? (
                        <>
                          ...
                          {!isSmsCampaign && !(isEmailCampaign && formData.emailBodyIsHtml) && renderRewriteContentEachRunOption()}
                        </>
                      ) : (
                        <>
                          {isMessageCampaign ? (
