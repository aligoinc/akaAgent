...
export function deviceChangeMessage(result: DeviceLockResetResult, source: 'login' | 'account_menu' = 'login'): string {
  switch (result.code) {
    case 'changed': return source === 'account_menu'
      ? 'Đổi máy tính thành công.'
      : `Đổi máy tính thành công. Bạn còn ${result.remainingChanges} lần đổi máy.`
    case 'already_unbound': return source === 'account_menu'
      ? 'Tài khoản chưa liên kết máy tính, không cần đổi máy.'
      : `Tài khoản chưa liên kết máy tính, không cần đổi máy. Bạn còn ${result.remainingChanges} lần đổi máy.`
