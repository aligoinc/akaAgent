...
async function seedElements(): Promise<void> {
  ...
  const elements: { name: string; xpath: string; description: string; category: string }[] = [
    ...
    { name: 'fb_composer_button', xpath: "//*[@role='main']//*[@role='button' and @tabindex='0' and not(@aria-disabled='true')] | //*[@role='button' and (contains(.,'Bạn đang nghĩ gì') or contains(.,'on your mind') or contains(.,'Write something') or contains(.,'Bạn viết gì đi'))]", description: 'Nút mở composer — timeline (CSS-based) hoặc group (text-based)', category: 'facebook' },
